...
export default function TourCard({ cardData }) {
    ...
    if (thumbnailURL) {
        ...
        if (/^data:image.*/.test(thumbnailURL))
            thumbnailDOM = <img src={thumbnailURL} style={{ objectPosition: poi }} />;
        else
            thumbnailDOM = (
                <>
    ...
    if (priceOriginal && priceOffered) {
        pricingDOM = (
            <>
                <span className='text-primary lead fw-medium'>{asCurrency(priceOffered)}</span>
                <small className={clsx(styles.price_old)}>{asCurrency(priceOriginal)}</small>
            </>
        );
    } else {
        pricingDOM = (
    ...
    let detailsDOM;
    if (details) {
        detailsDOM = <small className='card-text mb-auto text-wrap'>{details}</small>;
    } else {
        detailsDOM = (
    ...
    return (
        <div className={clsx(styles.card, "card border-0 h-100 gap-8")}>
            ...
            <div className={clsx(styles.card__body, "card-body d-flex flex-column gap-1")}>
                ...
                <div className='d-flex align-items-center gap-3 fw-medium my-3'>{pricingDOM}</div>
                {detailsDOM}
